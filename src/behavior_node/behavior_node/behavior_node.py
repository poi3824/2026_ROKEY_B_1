...
# 스테이션 좌표 (Isaac Sim World0123_stations.usd 기준, x/y[m], theta[rad]).
# station_1: 너트/배터리팩 체결 구역 - 현재 Nova_Carter 스폰 pose 그대로(검증됨, 팔이 nut1/nut2에 닿는 실제 위치).
# station_2: 버스바 삽입 구역 - bench_busbar 프림 pose로 추정(z가 바닥 기준점과 일치해 도킹 마커로 추정).
#            아직 실제 팔 도달 여부 미검증 - Isaac Sim에서 확인 후 조정 필요.
STATION_POSES = {
    'station_1': (0.667, -0.038, -1.5708),
    'station_2': (1.301, 1.807, 3.1416),
}

# station_1(배터리 트레이) 내부 6개 슬롯 순회 좌표 - 사용자가 그린 지그재그 순서(1->2->3->4->5->6)를
# 그대로 따름: Home(남쪽)에서 가장 가까운 줄부터 먼 열을 타고 올라갔다가(1->2->3),
# 가까운 열을 타고 다시 내려온다(4->5->6). AMR 정차는 x=0.667 고정, y만 Row에 맞춰 바뀐다.
# 실측 결과 한 Row에 서면 그 줄의 양쪽 열(near/far) 슬롯이 다 닿아서 slot_4/5/6는 slot_3/2/1과
# 정차 위치가 같다 - 그래도 작업 단위 구분을 위해 6개를 그대로 남겨둔다.
# 팔이 실제로 각 슬롯에 닿는지는 아직 검증 전 (사용자 지시로 이동 좌표만 먼저 확정, 도달 검증은 후속).
BATTERY_SLOT_POSES = {
    'slot_1': (0.667, -0.910, -1.5708),  # PackSlots/Slot_1 실좌표 (1.478, -0.910)
    'slot_2': (0.667, -0.385, -1.5708),  # PackSlots/Slot_2 실좌표 (1.478, -0.385)
    'slot_3': (0.667, 0.140, -1.5708),   # PackSlots/Slot_3 실좌표 (1.478, 0.140)
    'slot_4': (0.667, 0.140, -1.5708),   # PackSlots/Slot_6 실좌표 (1.178, 0.140)
    'slot_5': (0.667, -0.385, -1.5708),  # PackSlots/Slot_5 실좌표 (1.178, -0.385)
    'slot_6': (0.667, -0.910, -1.5708),  # PackSlots/Slot_4 실좌표 (1.178, -0.910)
}
