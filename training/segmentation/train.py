#!/usr/bin/env python3
"""Train a YOLOv8-seg model on the output of prepare_dataset.py.

The previous baseline (see training/README.md) was run with defaults left untouched
(30-100 epochs, no augmentation, light-occlusion data only) -- this wrapper exposes those
knobs explicitly instead of hiding them behind library defaults.

Usage:
    python prepare_dataset.py --occlusion both --cams color   # once, to build training/segmentation/data/
    python train.py --epochs 150 --model yolov8n-seg.pt
...
def main():
    ...
    ap.add_argument("--name", default="busbar_seg")
    ap.add_argument("--project", type=Path, default=DEFAULT_PROJECT)
    # augmentation -- left at ultralytics' generally-reasonable defaults, but exposed so
    # they're a deliberate choice instead of an accident. degrees/mixup/copy_paste are the
    # ones most likely to help with a rigid, always-upright object like the busbar.
    ap.add_argument("--degrees", type=float, default=15.0)
    ap.add_argument("--mixup", type=float, default=0.1)
    ap.add_argument("--copy-paste", type=float, default=0.1)
    args = ap.parse_args()
