...
# ══════════════════════════════════════════════════════════════════════════
USD_PATH = str(
    _THIS_DIR.parents[1] / "src" / "Collected_Busbar_fin" / "Busbar.usd"
)
...
FINGER_LINK_NAMES = ["left_inner_finger", "right_inner_finger"]
...
NUT_PEG_COLLIDER_PATHS = {
    1: "/World/Nova_Carter/chassis_link/carter_tray/peg_5",
    2: "/World/Nova_Carter/chassis_link/carter_tray/peg_4",
    3: "/World/Nova_Carter/chassis_link/carter_tray/peg_3",
    4: "/World/Nova_Carter/chassis_link/carter_tray/peg_1",
    5: "/World/Nova_Carter/chassis_link/carter_tray/peg_0",
    6: "/World/Nova_Carter/chassis_link/carter_tray/peg_2",
}

# 그리퍼 파라미터
GRIPPER_OPEN      = np.array([0.0, 0.0])
GRIPPER_CLOSE     = np.array([0.85, 0.85])
GRIPPER_CLOSE_NUT = np.array([0.97, 0.97])
GRIPPER_DELTA     = np.array([-0.5, -0.5])
GRIP_CLOSE_RAMP_STEPS = 75
NUT_PRECONTACT_POSITION = 0.90
NUT_CONTACT_PRIME_STEPS = 6  # 저강성 접촉을 0.1초 유지한 뒤 AMR 조인트 해제
NUT_AMR_RELEASE_SETTLE_STEPS = 1  # 실측: disable/remove 모두 다음 물리 스텝에 반영
NUT_FINAL_CLOSE_RAMP_STEPS = 50
NUT_HOLD_GAIN_RAMP_STEPS = 50
NUT_GRIP_HOLD_SETTLE_STEPS = 30  # 완전 조임 상태를 유지한 뒤 상승할 시간
NUT_LIFT_SPEED_M_S = 0.10  # 파지 직후 수직 상승 속도
GRIP_SETTLE_STEPS = 15  # 손가락이 다 닫힌 뒤 FixedJoint를 만들기 전 안정화 대기 틱 수
NUT_TARGET_RELOCALIZE_MAX_M = 0.050  # HOME 추정과 실제 너트가 이보다 다르면 잘못된 슬롯
NUT_GRASP_MAX_XY_ERROR = 0.008  # 실제 중심으로 재지정한 뒤 허용할 파지 중 수평 이탈 (8mm)
NUT_GRASP_MAX_Z_DROP_M = 0.010  # 조임 중 10mm 이상 떨어지면 접촉 실패
NUT_DESCEND_TOLERANCE_M = 0.004  # 실측 정지 오차 3.3mm를 허용하되 10mm 공통값은 쓰지 않음
NUT_DESCEND_TIMEOUT_STEPS = 900  # 물리 15초 기준이며 렌더링 때문에 실제 대기는 더 길 수 있음
NUT_LIFT_MIN_DELTA_Z = 0.050  # 실제 너트가 이 높이 이상 상승해야 파지 성공 (50mm)
NUT_MASS_KG = 0.015
NUT_STATIC_FRICTION = 1.0
NUT_DYNAMIC_FRICTION = 1.0
NUT_FINGER_STATIC_FRICTION = 1.8
NUT_FINGER_DYNAMIC_FRICTION = 1.4
NUT_CONTACT_OFFSET_M = 0.002
NUT_MAX_LINEAR_VELOCITY_M_S = 3.0
NUT_MAX_ANGULAR_VELOCITY_RAD_S = 20.0
NUT_MAX_DEPENETRATION_VELOCITY_M_S = 0.1
NUT_SOLVER_POSITION_ITERS = 192
NUT_SOLVER_VELOCITY_ITERS = 1
NUT_CLOSE_KP = 45.0
NUT_CLOSE_KD = 5.0
NUT_HOLD_KP = 180.0
NUT_HOLD_KD = 35.0
NUT_GRIP_MAX_FORCE = 50.0  # RG2 URDF의 각 관련 관절 effort limit
...
# 현재 Final 자산의 authored transform 실측에서 손가락 collider 하단은 EE보다
# 약 197mm 아래다. 너트 몸통(z≈0.497~0.506m)과 5mm가량 겹치도록 목표를 내린다.
NUT_PICK_Z_CORRECTION_M = 0.029
NUT_PICK_Z         = (
    NUT_SUPPLY_TABLE_Z
    - (NUT_GRASP_Z_LOCAL - 0.0395)
    - NUT_PICK_Z_CORRECTION_M
)
...
def _deinstance_subtree(stage, prim_path):
    """Instance proxy 안쪽 collider를 직접 조정할 수 있도록 참조를 해제한다."""
...
def _apply_contact_offsets(stage, root_path, contact_offset, rest_offset=0.0):
    """root 아래 모든 실제 collider에 명시적인 mm 단위 접촉 여유를 적용한다."""
...
def configure_gripper_contact_physics(stage):
    """성공한 nut grasp 실험의 손가락 offset/고마찰 재질을 적용한다."""
    material_path = "/World/Looks/NutFingerGripMaterial"
    material_prim = stage.GetPrimAtPath(material_path)
    if not material_prim.IsValid():
        material_prim = stage.DefinePrim(material_path, "Material")
    material_api = UsdPhysics.MaterialAPI.Apply(material_prim)
    material_api.CreateStaticFrictionAttr(
        NUT_FINGER_STATIC_FRICTION
    )
    material_api.CreateDynamicFrictionAttr(
        NUT_FINGER_DYNAMIC_FRICTION
    )
    ...
    for link_name in FINGER_LINK_NAMES:
        link_path = find_prim_path(stage, M0609_PATH, link_name)
        if link_path is None:
            raise RuntimeError(f"그리퍼 손가락 prim 누락: {link_name}")
        _deinstance_subtree(stage, link_path)
        count = _apply_contact_offsets(
            stage,
            link_path,
            NUT_CONTACT_OFFSET_M,
            0.0,
        )
        if count == 0:
            raise RuntimeError(f"그리퍼 손가락 collider 누락: {link_path}")
        for prim in Usd.PrimRange(stage.GetPrimAtPath(link_path)):
            if prim.HasAPI(UsdPhysics.CollisionAPI):
        ...
        total += count
    print(
        f"[GRIPPER PHYSICS] finger colliders={total}, "
        f"contactOffset={NUT_CONTACT_OFFSET_M*1000:.1f}mm, "
        f"friction={NUT_FINGER_STATIC_FRICTION}/"
        f"{NUT_FINGER_DYNAMIC_FRICTION}"
    )


def _nut_body_path_from_root(nut_root_path):
    return f"{nut_root_path}/geo/PolyShape"


def _finger_body_paths(stage):
    paths = []
    for link_name in FINGER_LINK_NAMES:
        path = find_prim_path(stage, M0609_PATH, link_name)
        if path is None:
            raise RuntimeError(f"그리퍼 손가락 prim 누락: {link_name}")
        paths.append(path)
    return paths
...
def enable_nut_arm_collision(stage, nut_root_path):
    """손가락 접촉이 시작부터 활성인지 확인한다.

    filteredPairs를 런타임에 제거하면 USD 관계는 바뀌어도 PhysX broadphase
    cache에는 이전 필터가 남을 수 있다. 따라서 PICK 중 관계를 바꾸지 않는다.
    """

    nut_body_path = _nut_body_path_from_root(nut_root_path)
    nut_body = stage.GetPrimAtPath(nut_body_path)
    if not nut_body.IsValid():
        print(f"[ERROR] 너트 rigid body prim 누락: {nut_body_path}")
        return False

    relation = UsdPhysics.FilteredPairsAPI(nut_body).GetFilteredPairsRel()
    finger_paths = {Sdf.Path(path) for path in _finger_body_paths(stage)}
    robot_targets = finger_paths | {Sdf.Path(M0609_PATH)}
    targets = set(relation.GetTargets()) if relation else set()
    blocked = targets.intersection(robot_targets)
    if blocked:
        print(
            f"[ERROR] {nut_body_path}: finger collision을 막는 "
            f"startup filter가 남아 있습니다: "
            f"{sorted(str(path) for path in blocked)}"
        )
        return False
    print(
        f"[NUT COLLISION] {nut_body_path}: runtime filter 변경 없음, "
        f"finger collision 차단 filter 없음; 유지={list(targets)}"
    )
    return True


def filter_nut_arm_collision(stage, nut_root_path):
    """호환 호출부: 런타임 필터를 추가하지 않고 활성 상태만 확인한다."""
    return enable_nut_arm_collision(stage, nut_root_path)


def configure_nut_contact_physics(stage):
    """PhysX 시작 전에 실제 nut rigid body에 접촉 물리를 적용한다."""

    material_path = "/World/Looks/NutGripMaterial"
    material_prim = stage.GetPrimAtPath(material_path)
    if not material_prim.IsValid():
        material_prim = stage.DefinePrim(material_path, "Material")
    material_api = UsdPhysics.MaterialAPI.Apply(material_prim)
    material_api.CreateStaticFrictionAttr(NUT_STATIC_FRICTION)
    material_api.CreateDynamicFrictionAttr(NUT_DYNAMIC_FRICTION)
    material_api.CreateRestitutionAttr(0.0)
    physx_material = PhysxSchema.PhysxMaterialAPI.Apply(material_prim)
    ...
    nut_pairs = [
        (1, NUT1_ROOT_PATH, NUT1_POLYSHAPE_PATH),
        (2, NUT2_ROOT_PATH, NUT2_POLYSHAPE_PATH),
        *[
            (index, root_path, body_path)
            for index, (root_path, body_path) in enumerate(
                zip(EXTRA_NUT_ROOT_PATHS, EXTRA_NUT_POLYSHAPE_PATHS),
                start=3,
            )
        ],
    ]
    for nut_index, nut_root_path, nut_body_path in nut_pairs:
        nut_root_prim = stage.GetPrimAtPath(nut_root_path)
        body_prim = stage.GetPrimAtPath(nut_body_path)
        if (
            not nut_root_prim.IsValid()
            or not body_prim.IsValid()
            or not body_prim.HasAPI(UsdPhysics.RigidBodyAPI)
            or not body_prim.HasAPI(UsdPhysics.CollisionAPI)
        ):
            raise RuntimeError(f"실제 너트 rigid body/collider 누락: {nut_body_path}")

        # 원본 Busbar.usd는 물리 body의 상위 /World/nutN에 m0609와
        # Nova_Carter 전체를 filteredPairs로 걸어 두었다. 이 ancestor 관계가
        # 하위 collider에도 적용되어 finger AABB가 겹쳐도 접촉력이 0이었다.
        # PhysX가 stage를 읽기 전에 상위 필터를 비우고, 아래 실제 body에는
        # 폭발 원인인 해당 peg 하나만 정밀하게 필터링한다.
        root_filtered_pairs = UsdPhysics.FilteredPairsAPI.Apply(
            nut_root_prim
        ).CreateFilteredPairsRel()
        removed_root_filters = list(root_filtered_pairs.GetTargets())
        root_filtered_pairs.SetTargets([])

        # 일부 공급 peg는 시작부터 nut mesh와 1mm 이상 겹친다. chassis 전체를
        # 필터링하면 트레이 바닥 지지까지 사라지므로 해당 슬롯의 peg shape만 뺀다.
        filtered_pairs = UsdPhysics.FilteredPairsAPI.Apply(
            body_prim
        ).CreateFilteredPairsRel()
        peg_path = Sdf.Path(NUT_PEG_COLLIDER_PATHS[nut_index])
        # PhysX가 stage를 읽기 전에 필터를 "해당 peg 하나"로 확정한다.
        # PICK 중 add/remove하지 않아 finger contact가 cache에 막히는 일을 없앤다.
        filtered_pairs.SetTargets([peg_path])

        UsdPhysics.MassAPI.Apply(body_prim).CreateMassAttr().Set(NUT_MASS_KG)
        mesh_collision = UsdPhysics.MeshCollisionAPI.Apply(body_prim)
        mesh_collision.CreateApproximationAttr().Set("sdf")
        PhysxSchema.PhysxSDFMeshCollisionAPI.Apply(
            body_prim
        ).CreateSdfResolutionAttr().Set(256)
        _apply_contact_offsets(
            stage,
            nut_body_path,
            NUT_CONTACT_OFFSET_M,
            0.0,
        ...
        physx_body.CreateMaxLinearVelocityAttr().Set(
            NUT_MAX_LINEAR_VELOCITY_M_S
        )
        physx_body.CreateMaxAngularVelocityAttr().Set(
            NUT_MAX_ANGULAR_VELOCITY_RAD_S
        )
        physx_body.CreateMaxDepenetrationVelocityAttr().Set(
            NUT_MAX_DEPENETRATION_VELOCITY_M_S
        )
        physx_body.CreateSolverPositionIterationCountAttr().Set(
            NUT_SOLVER_POSITION_ITERS
        )
        physx_body.CreateSolverVelocityIterationCountAttr().Set(
            NUT_SOLVER_VELOCITY_ITERS
        )
        ...
        print(
            f"[NUT PHYSICS] {nut_body_path}: mass={NUT_MASS_KG}kg, "
            f"friction={NUT_STATIC_FRICTION}/{NUT_DYNAMIC_FRICTION}, "
            f"contactOffset={NUT_CONTACT_OFFSET_M*1000:.1f}mm, "
            f"maxDepenetration={NUT_MAX_DEPENETRATION_VELOCITY_M_S:.1f}m/s, "
            f"solver={NUT_SOLVER_POSITION_ITERS}/{NUT_SOLVER_VELOCITY_ITERS}, "
            f"clearedAncestorFilters="
            f"{[str(path) for path in removed_root_filters]}, "
            f"filteredWith={peg_path}, sdf=256"
        )
...
def attach_nut_to_amr(stage, nut_root_path, nut_rigidbody_path, amr_link_path, nut_xform, robot, joint_name=NUT_AMR_JOINT_NAME):
    ...
    # chassis_link는 articulation root다. 운반용 너트를 articulation link로
    # 편입하면 runtime detach/remove가 제한되므로 이 외부 고정 조인트는
    # 처음부터 regular joint로 파싱되게 명시한다.
...
def detach_nut_from_amr(stage, nut_root_path, joint_name=NUT_AMR_JOINT_NAME):
    """AMR 고정 조인트를 비활성화한 뒤 제거해 너트를 완전히 풀어준다."""
    joint_path = f"{nut_root_path}/{joint_name}"
    joint_prim = stage.GetPrimAtPath(joint_path)
    ...
    stage.RemovePrim(joint_path)
    removed = not stage.GetPrimAtPath(joint_path).IsValid()
    print(f"[NUT JOINT DETACH] {joint_path} removed={removed}")
    return removed
...
def attach_nut_to_gripper(stage, nut_polyshape_path, gripper_link_path, robot, nut_xform):
    """너트를 현재 자세 그대로 그리퍼 EE 링크에 운반용 FixedJoint로 고정한다."""
    gripper_prim = stage.GetPrimAtPath(gripper_link_path)
    ...
    joint.CreateLocalPos0Attr().Set(
        Gf.Vec3f(*[float(v) for v in rel_pos])
    )
    joint.CreateLocalRot0Attr().Set(
        Gf.Quatf(
            float(rel_quat[0]),
            Gf.Vec3f(*[float(v) for v in rel_quat[1:]]),
        )
    )
    joint.CreateLocalPos1Attr().Set(Gf.Vec3f(0.0, 0.0, 0.0))
    joint.CreateLocalRot1Attr().Set(Gf.Quatf(1.0, 0.0, 0.0, 0.0))
...
def detach_nut_from_gripper(stage, nut_polyshape_path):
    ...
    if joint_prim.IsValid():
        UsdPhysics.FixedJoint(joint_prim).GetJointEnabledAttr().Set(False)
        stage.RemovePrim(joint_path)
        print(f"[NUT GRIP DETACH] {joint_path}")
...
def set_nut_gripper_gains(robot, stiffness, damping, label=None):
    """버스바 단계에서 높인 실제 구동 DOF만 너트용 안전 gain으로 복원한다.

    mimic/passive DOF까지 같은 양의 gain을 주면 음수 multiplier를 가진 관절이
    master의 닫힘을 거스르며 내부 힘을 만든다. 버스바 단계에서 직접 높인 두
    command DOF만 덮어쓰고 나머지 기구부 DOF는 원래 mimic 동작에 맡긴다.
    """
    available = [
        name for name in GRIPPER_JOINTS
        if name in robot.dof_names
    ]
    if not available:
        raise RuntimeError("너트 파지용 그리퍼 기구부 DOF를 찾지 못했습니다.")
    _set_runtime_gains(
        robot,
        available,
        stiffness,
    ...
    if label is not None:
        print(
            f"[NUT GRIP GAINS] {label}: joints={available}, "
            f"kp={stiffness:.1f}, kd={damping:.1f}, "
            f"max={NUT_GRIP_MAX_FORCE:.1f}"
        )


def gripper_joint_position_text(robot):
    positions = robot.get_joint_positions()
    values = []
    for name in GRIPPER_JOINTS:
        if name in robot.dof_names:
            values.append(
                f"{name}={float(positions[robot.get_dof_index(name)]):.4f}"
            )
    return ", ".join(values) if values else "unavailable"


def _world_bbox_min_max(stage, prim_path, cache=None):
    ...
    return (
        np.asarray(world_range.GetMin(), dtype=float),
        np.asarray(world_range.GetMax(), dtype=float),
    )


def log_nut_grasp_geometry(stage, nut_index, label):
    """파지 고도 디버깅용 실제 finger collider/nut bbox를 한 줄로 출력한다."""
    _, nut_body_path = nut_paths_for_index(nut_index)
    cache = UsdGeom.BBoxCache(
        Usd.TimeCode.Default(),
        [
            UsdGeom.Tokens.default_,
    ...
    nut_bounds = _world_bbox_min_max(stage, nut_body_path, cache)
    finger_parts = []
    for finger_path in _finger_body_paths(stage):
        collider_bounds = []
        for prim in Usd.PrimRange(stage.GetPrimAtPath(finger_path)):
            if prim.HasAPI(UsdPhysics.CollisionAPI):
                bounds = _world_bbox_min_max(stage, str(prim.GetPath()), cache)
                if bounds is not None:
                    collider_bounds.append(bounds)
        if collider_bounds:
            mins = np.min([bounds[0] for bounds in collider_bounds], axis=0)
            maxs = np.max([bounds[1] for bounds in collider_bounds], axis=0)
            center = 0.5 * (mins + maxs)
            finger_parts.append(
                f"{Path(finger_path).name}:"
                f"xy=({center[0]:.4f},{center[1]:.4f}),"
                f"x=[{mins[0]:.4f},{maxs[0]:.4f}],"
                f"y=[{mins[1]:.4f},{maxs[1]:.4f}],"
                f"z=[{mins[2]:.4f},{maxs[2]:.4f}]"
            )
    if nut_bounds is None:
        nut_text = "nut_bbox=unavailable"
    else:
        nut_min, nut_max = nut_bounds
        nut_center = 0.5 * (nut_min + nut_max)
        nut_text = (
            f"nut_xy=({nut_center[0]:.4f},{nut_center[1]:.4f}),"
            f"x=[{nut_min[0]:.4f},{nut_max[0]:.4f}],"
            f"y=[{nut_min[1]:.4f},{nut_max[1]:.4f}],"
            f"z=[{nut_min[2]:.4f},{nut_max[2]:.4f}]"
        )
    print(
        f"[NUT GEOMETRY] {label}: {nut_text}; "
        + "; ".join(finger_parts)
    )
...
def main():
    ...
    configure_gripper_contact_physics(stage)
    configure_nut_contact_physics(stage)
    ...
    if not ee_path:
        raise RuntimeError(f"[ERROR] End-effector prim 누락: {EE_LINK_NAME}")
    ...
    nut_grasp_start_pos = None  # 파지 시작 시 실제 너트 rigid-body world 좌표
    nut_lift_command_z = None  # 급상승을 막는 속도 제한 Z 명령
    ...
    while simulation_app.is_running():
        ...
        if playing and not was_playing:
            # hard reset은 stage를 PhysX에 다시 파싱한다. 외부 운반 조인트가
            # articulation topology에 들어가기 전에 모두 제거하고 reset 후 재생성한다.
            for nut_root_path in [
                NUT1_ROOT_PATH,
                NUT2_ROOT_PATH,
                *EXTRA_NUT_ROOT_PATHS,
            ]:
                remove_nut_amr_joint(stage, nut_root_path)
            detach_busbar_from_gripper(stage)
            for nut_poly_path in [
                NUT1_POLYSHAPE_PATH,
                NUT2_POLYSHAPE_PATH,
        ...
        if playing and isaac_node.requested_task:
            ...
            if task == "SCAN_BATTERY":
                ...
                if HOME_EE_POS is not None:
                    ...
                    # 손가락이 먼 상태에서 AMR 조인트부터 풀면 너트가 먼저 움직인다.
                    # finger contact는 시작부터 활성으로 두고, 저강성으로 접촉 직전까지
                    # 닫아 접촉을 정착시킨 다음 AMR 조인트를 푼다.
                    if not filter_nut_arm_collision(stage, nut_root_path):
                        print(
                            f"\n[ERROR] [{task}] 사전 파지용 너트-팔 충돌 "
                            "필터를 설정하지 못했습니다."
                        )
                        publish_status("FAILURE:NUT_COLLISION_SETUP")
                    else:
                        nut_offset = NUT_OFFSET_FROM_HOME[nut_index]
                        nominal_x = HOME_EE_POS[0] + nut_offset[0]
                        nominal_y = HOME_EE_POS[1] + nut_offset[1]
                        ...
                        if nut_xf_cur is None:
                            print(f"\n[ERROR] [{task}] 너트 rigid-body prim을 찾지 못했습니다.")
                            publish_status("FAILURE:NUT_BODY_MISSING")
                        else:
                            actual_nut_pos, _ = nut_xf_cur.get_world_pose()
                            nut_grasp_start_pos = np.asarray(
                                actual_nut_pos,
                                dtype=float,
                            )
                            nominal_xy_error = math.hypot(
                                nominal_x - nut_grasp_start_pos[0],
                                nominal_y - nut_grasp_start_pos[1],
                            )
                            print(
                                f"[NUT TARGET CHECK] nut={nut_index} "
                                f"home_estimate=({nominal_x:.4f},"
                                f"{nominal_y:.4f}) actual=("
                                f"{nut_grasp_start_pos[0]:.4f},"
                                f"{nut_grasp_start_pos[1]:.4f}) "
                                f"correction={nominal_xy_error*1000:.1f}mm"
                            )
                            if nominal_xy_error > NUT_TARGET_RELOCALIZE_MAX_M:
                                print(
                                    f"\n[ERROR] [{task}] 선택한 너트가 HOME 추정에서 "
                                    f"{nominal_xy_error*1000:.1f}mm 벗어나 슬롯을 "
                                    "신뢰할 수 없습니다."
                                )
                                publish_status("FAILURE:NUT_TARGET_MISMATCH")
                            else:
                                # 11-19 실행은 하드코딩 추정이 실제 중심에서 19.1mm
                                # 벗어나 너트 가장자리를 쳤다. 선택된 rigid body의 현재
                                # 중심을 이 작업의 스냅샷 목표로 사용한다.
                                pick_x = float(nut_grasp_start_pos[0])
                                pick_y = float(nut_grasp_start_pos[1])
                                nut_pick_pos = np.array([
                                    pick_x,
                                    pick_y,
                                    NUT_PICK_Z,
                                ])
                                nut_approach_pos = np.array([
                                    pick_x,
                                    pick_y,
                                    NUT_APPROACH_Z,
                                ])
                                set_nut_gripper_gains(
                                    robot,
                                    NUT_CLOSE_KP,
                                    NUT_CLOSE_KD,
                                    label="파지 전 저강성 복원",
                                ...
                                step_count = 0
                                print(
                                    f"\n>>> [{task}] 너트 {nut_index}번 실제 중심 "
                                    f"상공 접근 시작 (Target: X={pick_x:.3f}, "
                                    f"Y={pick_y:.3f}, pick Z={NUT_PICK_Z:.3f})"
                                )
                else:
                    print(f"\n[ERROR] [{task}] 초기 위치(HOME_EE_POS)가 없습니다. 먼저 INIT_POSE가 수행되어야 합니다.")
                    publish_status("FAILURE:NO_HOME_POSE")
        ...
        if playing and phase != "IDLE" and phase != "DONE":
            ...
            # [12단계] 너트 파지점 하강
                ...
                if step_count % 30 == 0:
                    print(
                        f"[NUT DESCEND] EE=({cur_pos[0]:.4f},"
                        f"{cur_pos[1]:.4f},{cur_pos[2]:.4f}) "
                        f"target=({nut_pick_pos[0]:.4f},"
                        f"{nut_pick_pos[1]:.4f},{nut_pick_pos[2]:.4f}) "
                        f"error={current_err*1000:.1f}mm"
                    )

                if current_err < NUT_DESCEND_TOLERANCE_M:
                    nut_label = resolve_nut_assets(nut_index, nut1_xform, nut2_xform)[1]
                    log_nut_grasp_geometry(stage, nut_index, "하강 완료/열림")
                    print(f"[OK] {nut_label} 하강 완료! -> 물리 파지(Gripper Close) 시작")
                    phase = "NUT_GRASP"
                    grasp_timer = 0
                elif step_count >= NUT_DESCEND_TIMEOUT_STEPS:
                    nut_label = resolve_nut_assets(
                        nut_index, nut1_xform, nut2_xform
                    )[1]
                    log_nut_grasp_geometry(stage, nut_index, "하강 수렴 실패")
                    print(
                        f"[ERROR] {nut_label} 하강이 "
                        f"{NUT_DESCEND_TIMEOUT_STEPS * PHYSICS_DT:.1f}초 안에 "
                        f"{NUT_DESCEND_TOLERANCE_M*1000:.1f}mm로 수렴하지 "
                        f"못했습니다 (최종 오차 {current_err*1000:.1f}mm)."
                    )
                    publish_status("FAILURE:NUT_DESCEND_NOT_CONVERGED")
                    phase = "IDLE"
            ...
            # [13단계] 너트 물리 파지
                ...
                grasp_timer += 1
                # AMR FixedJoint가 살아 있는 동안 손가락을 실제 접촉 직전
                # (0.90rad)까지만 저강성으로 천천히 가져간다. finger collision은
                # startup부터 활성이라 런타임 broadphase filter 갱신에 의존하지 않는다.
                ramp_frac = min(grasp_timer / GRIP_CLOSE_RAMP_STEPS, 1.0)
                grip_target = ramp_frac * NUT_PRECONTACT_POSITION
                ...
                if grasp_timer >= GRIP_CLOSE_RAMP_STEPS:
                    nut_root_path, _ = nut_paths_for_index(nut_index)
                    if not enable_nut_arm_collision(stage, nut_root_path):
                        print("[ERROR] 너트-손가락 충돌 활성화 실패")
                        publish_status("FAILURE:NUT_COLLISION_SETUP")
                        phase = "IDLE"
                    else:
                        nut_release_timer = 0
                        print(
                            "[NUT] 0.90rad 사전 닫기 완료 -> 실제 finger "
                            "collision filter 확인, 저강성 정착"
                        )
                        phase = "NUT_CONTACT_PRIME"

            # FixedJoint를 풀기 전에 저강성 사전 닫기를 0.1초 유지한다.
            # 이 대기는 실제 contact sensor 판정이 아니므로 로그에서 접촉을 단정하지 않는다.
            elif phase == "NUT_CONTACT_PRIME":
                publish_progress("NUT_CONTACT_PRIME", 65.0)
                actions = arm_controller.forward(
                    target_end_effector_position=nut_pick_pos,
                ...
                robot.gripper.apply_action(
                    ArticulationAction(
                        joint_positions=np.array([
                            NUT_PRECONTACT_POSITION,
                            NUT_PRECONTACT_POSITION,
                        ])
                    )
                )
                nut_release_timer += 1
                if nut_release_timer >= NUT_CONTACT_PRIME_STEPS:
                    nut_root_path, _ = nut_paths_for_index(nut_index)
                    if not detach_nut_from_amr(stage, nut_root_path):
                        print("[ERROR] AMR-너트 FixedJoint 해제 실패")
                        publish_status("FAILURE:NUT_AMR_DETACH")
                        phase = "IDLE"
                    else:
                        nut_release_timer = 0
                        print(
                            "[NUT] 사전 닫기 유지 완료 -> AMR FixedJoint 해제 "
                            "(실제 contact sensor 검증은 아직 없음)"
                        )
                        phase = "NUT_GRASP_SETTLE"

            # [13.5단계] 조인트 해제 1스텝 반영 -> 실제 접촉 조임 -> 정지 상태 gain 램프
                ...
                nut_release_timer += 1
                close_ramp_start = NUT_AMR_RELEASE_SETTLE_STEPS
                close_ramp_end = close_ramp_start + NUT_FINAL_CLOSE_RAMP_STEPS
                hold_gain_ramp_end = close_ramp_end + NUT_HOLD_GAIN_RAMP_STEPS
                if nut_release_timer <= close_ramp_start:
                    grip_target = NUT_PRECONTACT_POSITION
                elif nut_release_timer <= close_ramp_end:
                    close_fraction = (
                        (nut_release_timer - close_ramp_start)
                        / NUT_FINAL_CLOSE_RAMP_STEPS
                    )
                    grip_target = (
                        NUT_PRECONTACT_POSITION
                        + close_fraction
                        * (GRIPPER_CLOSE_NUT[0] - NUT_PRECONTACT_POSITION)
                    )
                else:
                    grip_target = GRIPPER_CLOSE_NUT[0]
                robot.gripper.apply_action(
                    ArticulationAction(
                        joint_positions=np.array([grip_target, grip_target])
                    )
                ...
                if nut_release_timer == close_ramp_start:
                    print(
                        "[NUT] AMR 조인트 해제 1스텝 반영 -> "
                        f"{GRIPPER_CLOSE_NUT[0]:.2f}rad까지 천천히 조임"
                    )
                elif nut_release_timer == close_ramp_end:
                    log_nut_grasp_geometry(stage, nut_index, "0.97rad 명령 완료")
                    print(
                        f"[NUT] {GRIPPER_CLOSE_NUT[0]:.2f}rad 목표 명령 완료 "
                        f"(실제: {gripper_joint_position_text(robot)}) -> "
                        "정지 상태에서 유지 gain 램프"
                    )

                if nut_release_timer > close_ramp_end:
                    gain_fraction = min(
                        (nut_release_timer - close_ramp_end)
                        / NUT_HOLD_GAIN_RAMP_STEPS,
                        1.0,
                    )
                    hold_kp = (
                        NUT_CLOSE_KP
                        + gain_fraction * (NUT_HOLD_KP - NUT_CLOSE_KP)
                    )
                    hold_kd = (
                        NUT_CLOSE_KD
                        + gain_fraction * (NUT_HOLD_KD - NUT_CLOSE_KD)
                    )
                    set_nut_gripper_gains(robot, hold_kp, hold_kd)
                    if nut_release_timer == hold_gain_ramp_end:
                        print(
                            "[NUT GRIP GAINS] 유지 gain 도달: "
                            f"kp={NUT_HOLD_KP:.1f}, kd={NUT_HOLD_KD:.1f}; "
                            f"actual={gripper_joint_position_text(robot)}"
                        )
                ...
                if nut_release_timer >= hold_gain_ramp_end:
                    ...
                    if nut_xf_cur is None:
                        print(f"[ERROR] {nut_label} rigid-body prim 누락")
                        publish_status("FAILURE:NUT_BODY_MISSING")
                        phase = "IDLE"
                    else:
                        current_nut_pos, _ = nut_xf_cur.get_world_pose()
                        current_nut_pos = np.asarray(current_nut_pos, dtype=float)
                        target_xy_error = math.hypot(
                            nut_pick_pos[0] - current_nut_pos[0],
                            nut_pick_pos[1] - current_nut_pos[1],
                        )
                        z_drop = float(
                            nut_grasp_start_pos[2] - current_nut_pos[2]
                        )
                        if target_xy_error > NUT_GRASP_MAX_XY_ERROR:
                            print(
                                f"[ERROR] {nut_label}가 조임 중 파지점에서 "
                                f"{target_xy_error*1000:.1f}mm 이탈했습니다."
                            )
                            publish_status("FAILURE:NUT_GRASP_MISALIGNED")
                            phase = "IDLE"
                            continue
                        if z_drop > NUT_GRASP_MAX_Z_DROP_M:
                            print(
                                f"[ERROR] {nut_label}가 조임 중 "
                                f"{z_drop*1000:.1f}mm 낙하했습니다. "
                                "손가락 높이/접촉이 맞지 않습니다."
                            )
                            publish_status("FAILURE:NUT_GRASP_NO_CONTACT")
                            phase = "IDLE"
                            continue

                        print(
                            f"[OK] {nut_label} 물리 접촉 파지 자세 확인 -> "
                            f"목표 {GRIPPER_CLOSE_NUT[0]:.2f}rad 유지 "
                            f"(실제: {gripper_joint_position_text(robot)})"
                        )
                        phase = "NUT_GRIP_HOLD_SETTLE"
                        nut_release_timer = 0

            # [13.75단계] 순수 물리 접촉 파지를 정지 상태로 안정화한다.
            elif phase == "NUT_GRIP_HOLD_SETTLE":
                publish_progress("NUT_GRIP_HOLD_SETTLE", 75.0)
                actions = arm_controller.forward(
                    target_end_effector_position=nut_pick_pos,
                    target_end_effector_orientation=quat_nut,
                )
                ...
                nut_release_timer += 1
                if nut_release_timer >= NUT_GRIP_HOLD_SETTLE_STEPS:
                    cur_pos = world_xf(
                        stage,
                    ...
                    print(
                        f"[NUT] 물리 접촉 파지 안정화 완료 -> "
                        f"{NUT_LIFT_SPEED_M_S:.2f}m/s로 상승"
                    )
                    phase = "NUT_LIFT"
                    step_count = 0
            ...
            # [14단계] 너트 파지 후 상공 상승
                ...
                nut_lift_command_z = min(
                    nut_lift_command_z + NUT_LIFT_SPEED_M_S * PHYSICS_DT,
                    nut_approach_pos[2],
                )
                lift_step_pos = np.array([
                    nut_approach_pos[0],
                    nut_approach_pos[1],
                    nut_lift_command_z,
                ])
                actions = arm_controller.forward(target_end_effector_position=lift_step_pos, target_end_effector_orientation=quat_nut)
                robot.apply_action(actions)
                ...
                if current_err < PICK_TOLERANCE_STRICT or (current_err < PICK_TOLERANCE_LOOSE_VAL and step_count > MAX_STUCK_STEPS):
                    ...
                    actual_nut_pos = (
                        np.asarray(nut_xf_cur.get_world_pose()[0], dtype=float)
                        if nut_xf_cur is not None
                        else None
                    )
                    lift_delta_z = (
                        float(actual_nut_pos[2] - nut_grasp_start_pos[2])
                        if actual_nut_pos is not None
                        and nut_grasp_start_pos is not None
                        else float("-inf")
                    )
                    print(
                        f"[NUT LIFT CHECK] {nut_label} actual_z="
                        f"{actual_nut_pos[2]:.4f}m delta_z="
                        f"{lift_delta_z*1000:.1f}mm"
                        if actual_nut_pos is not None
                        else f"[NUT LIFT CHECK] {nut_label} pose 확인 실패"
                    )
                    if lift_delta_z < NUT_LIFT_MIN_DELTA_Z:
                        print(
                            f"\n[ERROR] {nut_label} 실제 상승량이 "
                            f"{NUT_LIFT_MIN_DELTA_Z*1000:.0f}mm 미만입니다."
                        )
                        publish_status("FAILURE:NUT_NOT_LIFTED")
                    else:
                        print(f"\n★ [PICK_{nut_label} SUCCESS] 실제 너트 파지 및 상승 확인!")
