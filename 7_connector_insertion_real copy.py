...
INSERT_DEPTH    = 0.005                 # 캐비티 입구보다 이만큼 더 내림
...
def main():
    ...
    # 소켓(리셉터클): 고정, 정확 삼각메시(오목 캐비티)
    reference_part("/World/receptacle", RECEP_USD,
                   [RECEP_XY[0], RECEP_XY[1], RECEP_Z], RECEP_EULER, PART_SCALE)
    ...
    place_pos = np.array([rctr[0], rctr[1], rmx[2] + grasp_to_mating - INSERT_DEPTH])
    recep_center_xy = rctr[:2].copy()
